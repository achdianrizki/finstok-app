...
class ReturnPurchaseItemsExport implements FromCollection, WithHeadings, WithStyles, WithEvents
{
    public function collection()
    {
        return ReturnPurchase::with(['purchase', 'supplier', 'items'])->get()->flatMap(function ($returnPurchase) {
            return $returnPurchase->items->map(function ($item) use ($returnPurchase) {
                return [
                    'return_date'     => \Carbon\Carbon::parse($returnPurchase->return_date)->format('d-m-Y'),
                    'purchase_id'     => $returnPurchase->purchase->purchase_number,
                    'supplier_id'     => $returnPurchase->supplier->name,
                    'item_id'         => $item->name,
                    'qty'             => $item->pivot->qty,
                    'unit'            => $item->unit,
    ...
    {
        return ['Tanggal', 'Nomor Pembelian', 'Nama Supplier', 'Nama Barang', 'Jumlah Retur', 'Satuan', 'Alasan', 'Harga Beli'];
    }
