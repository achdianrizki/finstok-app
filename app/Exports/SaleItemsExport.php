...
class SaleItemsExport implements FromCollection, WithHeadings, WithStyles, WithEvents
{
    protected $period;
    protected $startDate;
    protected $endDate;
        ...
        return $this->sales->flatMap(function ($sale) {
            return $sale->items->map(function ($item) use ($sale) {
                return [
                    ...
                    'item_id'         => $item->name,
                    'qty_sold'        => $item->pivot->qty_sold ?? 0,
                    'unit'            => $item->unit,
                    'discount1'       => (float) $item->pivot->discount1 ?? 0,
                    'discount2'       => (float) $item->pivot->discount2 ?? 0,
                    'discount3'       => (float) $item->pivot->discount3 ?? 0,
                    'ad'              => (string) $item->pivot->ad ?? 0,
                    'sale_price'      => $item->pivot->sale_price ?? 0,
    ...
    public function headings(): array
    {
        ...
        return ['Tanggal', 'Nomor Pembelian', 'Nama Pelanggan', 'Nama Barang', 'Jumlah', 'Satuan', 'Diskon 1', 'Diskon 2', 'Diskon 3', 'Ad', 'Harga Jual'];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $totalQty = $this->sales->flatMap(function ($sale) {
                    return $sale->items->map(function ($item) {
                        return ($item->pivot->qty_sold ?? 0);
                    });
                })->sum();
                ...
                $totalDiscount = $this->sales->sum(function ($sale) {
                    return 
                        ($sale->discount1_value ?? 0) + 
                        ($sale->discount2_value ?? 0) + 
                        ($sale->discount3_value ?? 0);
                });

                $totalSalePrice = $this->sales->flatMap(function ($sale) {
                    return $sale->items->map(function ($item) {
                        return ($item->pivot->qty_sold ?? 0) * ($item->pivot->sale_price ?? 0);
                    });
                })->sum();
                ...
                $formulaColumn = 'K';
                $sheet->setCellValue("{$formulaColumn}{$totalRow}", $totalSalePrice - $totalDiscount);
