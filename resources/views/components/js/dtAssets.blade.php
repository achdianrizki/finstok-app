<script>
    $(document).ready(function() {
        ...
        function fetchdistributors(page, searchQuery = '') {
            ...
            $.ajax({
                url: '/assets-data?page=' + page + '&search=' + searchQuery,
                method: 'GET',
                ...
                `;
                        ...
                        <tr class="border dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-slate-900">
                              ...
                              <td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">${formatRupiah(asset.price)}</td>
                              <td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">${asset.qty}</td>
                              <td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">${formatRupiah(asset.total_price)}</td>
                              <td class="px-6 py-4 whitespace-nowrap">
                                  <x-button target="" href="/manager/asset/${asset.id}/edit" variant="warning" class="justify-center max-w-sm gap-2">
                                      <x-heroicon-o-pencil class="w-3 h-3" aria-hidden="true" />
                                  </x-button>
                                  <!-- Destroy form -->
                                      <form action="/manager/asset/${asset.id}" method="POST" class="inline-block">
                                          @csrf
                                          @method('DELETE')
                          ...
                          <tr id="details-${index}" class="hidden sm:hidden">
                              <td colspan="6" class="px-6 py-4">
                                  <div>
                                      <p><strong>Alamat:</strong> ${asset.name}</p>
                                      <p><strong>Nomor telepon:</strong> ${asset.price}</p>
                                  </div>
                              </td>
                          </tr>
                    `;
