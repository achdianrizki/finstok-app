<script>
  $(document).ready(function() {
      ...
      function fetchitems(page, searchQuery = '') {
          ...
          $.ajax({
              url: '/incomingpayment-data?page=' + page + '&search=' + searchQuery,
              method: 'GET',
                  ...
                  if (response.data.length === 0) {
                      ...
                      $.each(response.data, function(index, sale) {
                          ...
                          <tr class="border dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-slate-900">
                              ...
                              <td class="px-6 py-4 whitespace-nowrap">
                                  <x-button target="" href="/manager/purchase/${sale.id}/edit" variant="warning" class="justify-center max-w-sm gap-2">
                                      <x-heroicon-o-pencil class="w-3 h-3" aria-hidden="true" />
                                  </x-button>
                                  <form method="POST" action="/manager/purchases/${sale.id}" style="display:inline;">
                                      @csrf
