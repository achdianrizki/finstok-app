<script>
  $(document).ready(function() {
      ...
      function fetchitems(page, searchQuery = '') {
          ...
          $.ajax({
              url: '/report-return-sale-items?page=' + page + '&search=' + searchQuery,
              method: 'GET',
              success: function(response) {
                  ...
                  if (response.data.length === 0) {
                      ...
                      $.each(response.data, function(index, returnSale) {
                          const returnDate = new Date(returnSale.return_date);
                          ...
                          rows += `
                            <tr class="border dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-slate-900">
                              <td class="px-6 py-4 whitespace-nowrap">${ returnSale.sale_number }</td>
                              <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell">${formattedDate}</td>
                              <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell">${ returnSale.buyer_name }</td>
                              <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell">${ returnSale.qty }</td>
                            </tr>
                                <tr id="details-${index}" class="hidden sm:hidden">
                                        <td colspan="6" class="px-6 py-4">
                                                <div>
                                                        <p><strong>No Penjualan:</strong> ${returnSale.sale_number}</p>
                                                        <p><strong>Jumlah Retur:</strong> ${returnSale.qty}</p>
                                                </div>
                                        </td>
                                </tr>
                                    `;
