        ...
        <tr>
                ...
                <table class="table information" style="width: 400px">
                    ...
                    <tr>
                        ...
                        <td style="border-left: none;">
                            <p style="margin-top: -5px">: {{ $sale->buyer->address }}</p>
                            <p>: {{ $sale->buyer?->NPWP ?? '' }}</p>
                            <p>: </p>
            ...
            <td rowspan="2" style="border-top: none; border-bottom: none; border-left: none; border-right: none;">
                <table class="table" style="width: 280px; margin-top: -2px;">
                    <tr>
                        <td style="font-size: 14px;" colspan="2">Faktur Penjualan</td>
                    </tr>
                    <tr>
                        <td style="width: 50%;">
                        ...
                        <td style="width: 50%;">
                            ...
                            <p style="text-align: right;">{{ $sale->sale_number }}</p>
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 50%;">
                            <p style=" margin-top: -5px;">Jatuh Tempo</p>
                            <p style="text-align: right; margin-bottom: 5px;">{{ \Carbon\Carbon::parse($sale->due_date)->translatedFormat('d-F-Y') }}
                            </p>
                        </td>
                        <td style="width: 50%;">
                    ...
                    <tr>
                        <td style="width: 50%;">
                            ...
                            <p style="text-align: right;">{{ $sale->salesman?->name ?? '' }}</p>
                        </td>
                        <td style="width: 50%;">
    ...
    <table class="table" style="margin-top: 5px">
        ...
        @foreach ($sale->items as $item)
            <tr class="td-none">
                ...
                <td style="text-align: right">{{ $item->pivot->qty_sold }}</td>
                <td style="text-align: left">{{ $item->unit }}</td>
                <td style="text-align: right">Rp {{ number_format($item->pivot->sale_price, 2, ',', '.') }}

                </td>
                <td style="text-align: right">{{ number_format($item->pivot->discount1, 2) }}</td>
                <td style="text-align: right">{{ number_format($item->pivot->discount2, 2) }}</td>
                ...
                <td style="text-align: right">Rp {{ number_format($item->pivot->qty_sold * $item->pivot->sale_price, 2, ',', '.') }}
                </td>
            </tr>
        @endforeach
        <tr>
            <td style="text-align: center; font-size: 12px;" colspan="2">Total</td>
            <td style="text-align: right; padding: 3px;"> {{ $sale->items->sum('pivot.qty_sold') }}</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
    ...
    <table class="table" style="margin-top: -1px">
        <tr>
            ...
            <td style="width: 170px; border-left: none">
                <div class="total-section">
                    <p style="text-align: right; margin-top: -2px;">Rp {{ number_format($sale->sub_total, 2, ',', '.') }}</p>
                    <p style="text-align: right;">Rp {{ number_format($sale->discount1_value, 2, ',', '.') }}</p>
                    <p style="text-align: right;">Rp {{ number_format($sale->discount2_value, 2, ',', '.') }}</p>
                    <p style="text-align: right;">Rp {{ number_format($sale->discount3_value, 2, ',', '.') }}</p>
                    <p style="text-align: right;">Rp {{ number_format($sale->tax, 2, ',', '.') }}</p>
                    <p style="text-align: right;">Rp {{ number_format($sale->total_price, 2, ',', '.') }}</p>
                </div>
            </td>
        </tr>
    </table>
