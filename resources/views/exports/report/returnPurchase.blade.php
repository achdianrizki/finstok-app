...
<body>
    ...
    <div class="header">
        ...
        <div class="title-container">
            <p class="title">LAPORAN RETUR PEMBELIAN</p>
            {{-- <p class="period">
                Periode:
                @if ($period === 'day')
    ...
    <table class="table">
        ...
        @forelse ($returnPurchases as $singlePurchase)
            @foreach ($singlePurchase->items as $item)
                <tr>
                    <td class="text-center">{{ \Carbon\Carbon::parse($singlePurchase->return_date)->format('d/m/Y') }}</td>
                    <td class="text-center">{{ $singlePurchase->purchase->purchase_number }}</td>
                    <td class="text-center">{{ $singlePurchase->supplier->name }}</td>
                    <td>{{ $item->name }}</td>
                    <td class="text-center">{{ $item->pivot->qty }}</td>
                    <td>{{ $item->unit }}</td>
                    <td>{{ $singlePurchase->reason }}</td>
                    <td class="text-right">Rp {{ number_format($item->pivot->price_per_item, 2, ',', '.') }}</td>
                    <td class="text-right">Rp {{ number_format($singlePurchase->total_return, 2, ',', '.') }}</td>
                </tr>
            @endforeach
        @empty
            <tr>
