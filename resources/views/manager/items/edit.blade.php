<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <x-button href="{{ route('manager.items.index') }}" variant="primary" size="sm" class="justify-center gap-2">
    ...
    <form action="{{ route('manager.items.update', $item->id) }}" method="POST">
        ...
        <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <x-form.input id="name" class="w-full" type="text" name="name" value="{{ old('name', $item->name) }}" placeholder="Nama Barang" />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />

                    <x-form.label for="code" :value="__('Kode Barang')" />
                    <x-form.input id="code" class="w-full" type="text" name="code" value="{{ old('code', $item->code) }}" disabled />

                    <x-form.label for="unit" :value="__('Satuan')" />
                    <x-form.input id="unit" class="block w-full" type="text" name="unit" value="{{ old('unit', $item->unit) }}" placeholder="Satuan (pcs, kg, liter, dll)" />
                    <x-input-error :messages="$errors->get('unit')" class="mt-2" />

                    <x-form.label for="description" :value="__('Deskripsi')" />
                    <textarea id="description" name="description" class="w-full border-gray-400 rounded-md" rows="3" placeholder="Deskripsi barang">{{ old('description', $item->description) }}</textarea>
                    <x-input-error :messages="$errors->get('description')" class="mt-2" />
                </div>

                <div class="space-y-2">
                    <x-form.label for="purchase_price" :value="__('Harga Beli')" />
                    <x-form.input id="purchase_price" class="w-full" type="number" name="purchase_price" value="{{ old('purchase_price', intval($item->purchase_price)) }}" placeholder="Harga Beli" />
        ...
        <div class="mt-5 space-y-2">
            ...
            <button type="button" id="addSupplierRow" class="mt-2 px-4 py-2 bg-purple-500 text-white rounded">+ Tambah Supplier</button>
                            ...
                            <td class="p-2">
                                <select name="suppliers[]" class="w-full p-2 border border-gray-300 rounded">
                                    @foreach ($suppliers as $sup)
                                        <option value="{{ $sup->id }}" {{ $supplier->id == $sup->id ? 'selected' : '' }}>{{ $sup->contact }}</option>
                                    @endforeach
                                </select>
                            </td>
