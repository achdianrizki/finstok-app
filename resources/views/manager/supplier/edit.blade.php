<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <x-button target="" href="{{ route('manager.supplier.index') }}" variant="primary" size="sm"
    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <form action="{{ route('manager.supplier.update', $supplier->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <x-form.input id="supplier_code" class="block w-full" type="text" name="supplier_code"
                        :value="old('supplier_code', $supplier->supplier_code)" placeholder="{{ __('Masukan Kode Pemasok') }}" />
                    @error('supplier_code')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror

                    <x-form.label for="name" :value="__('Nama Pemasok')" />
                    <x-form.input id="name" class="block w-full" type="text" name="name" :value="old('name', $supplier->name)"
                    ...
                    <x-form.input id="contact" class="block w-full" type="text" name="contact" :value="old('contact', $supplier->contact)"
                    ...
                    <x-form.input id="phone" class="block w-full" type="text" name="phone" :value="old('phone', $supplier->phone)"
                ...
                <div class="space-y-2">
                    ...
                    <x-form.input id="fax_nomor" class="block w-full" type="text" name="fax_nomor" :value="old('fax_nomor', $supplier->fax_nomor)"
                    ...
                    <x-form.input id="city" class="block w-full" type="text" name="city" :value="old('city', $supplier->city)"
                    ...
                    <x-form.input id="province" class="block w-full" type="text" name="province" :value="old('province', $supplier->province)"
                    ...
                    <x-form.input id="payment_term" class="block w-full" type="text" name="payment_term"
                        :value="old('payment_term', $supplier->payment_term)" placeholder="{{ __('Masukan Jangka Waktu Pembayaran') }}" />
                    @error('payment_term')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
